...
namespace Universal.EBI.Responsible.API.Services
{
    public class RegisterResponsibleIntegrationHandler : BackgroundService
    {
        ...
        {
            _bus.RespondAsync<RegisteredResponsibleIntegrationEvent, ResponseMessage>(async request => await RegisterChild(request));
        ...
        private async Task<ResponseMessage> RegisterChild(RegisteredResponsibleIntegrationEvent message)
        {            
            var ChildCommand = new RegisterResponsibleCommand
            {
